...
const grahamCrackerGroupList = document.querySelector(`.graham-cracker-group-list`);
const recipeInstructionList = document.querySelector(`.recipe-instruction__list`);
const cheeseCakeGroupList = document.querySelector(`.cheesecake-group-list`);

const renderGrahamCracker = () => {
    grahamCrackerData.forEach((grahamCrackerDataItem, index) => {
        grahamCrackerGroupList.innerHTML += `
            <label class="checkbox-container recipe-ingredients-group-list__item">${grahamCrackerDataItem}
                <input type="checkbox">
...
const rendercheeseCake = () => {
    cheeseCakeData.forEach((cheeseCakeDataItem, index) => {
        cheeseCakeGroupList.innerHTML += `
            <label class="checkbox-container recipe-ingredients-group-list__item">${cheeseCakeDataItem}
                <input type="checkbox">
                <span class="checkmark"></span>
            </label>
        `
...
const renderRecipeInstruction = () => {
    instructionData.forEach((instructionDataItem, index) => {
        recipeInstructionList.innerHTML += `
            <div class="recipe-instruction-list__item">
                <div class="recipe-instruction-list-item__index">${index+1}</div>
                <div class="recipe-instruction-list-item__content">${instructionDataItem}</div>
            </div>
        `
    })
}
